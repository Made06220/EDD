...
CCapteurGps::CCapteurGps(QObject *parent, const char *nomvs, int no, int vit, char par, int nbc):
      QThread(parent)
  {
...
void CCapteurGps::stop()
{
  mArret=true;
  while(!mStopped);
}

int CCapteurGps::lireMesure(QByteArray &latitude, QByteArray &longitude)
{
  QByteArray ligne;
